...
if __name__ == "__main__":
    ...
    input_t = (
        3  # number of input time points to sample, only relevant for SEN12MS-CR-TS
    )
    import_path = None  # path to importing the suppl. file specifying what time points to load for input and output
    sample_type = (
        "cloudy_cloudfree"  # type of samples returned [cloudy_cloudfree | generic]
    )
    ...
    dataloader = torch.utils.data.DataLoader(
        loader, batch_size=1, shuffle=False, num_workers=10
    )
