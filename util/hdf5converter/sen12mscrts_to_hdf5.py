...
class SEN12MSCRTS_to_hdf5(SEN12MSCRTS):
    def __init__(
        ...
        if self.region == "all":
            ...
            ]  # insert your favorite validation split here
            self.splits["train"] = [
                roi
                for roi in all_ROI
    ...
    def get_cloud_mask(self, img, mask_type):
        if mask_type == "cloud_cloudshadow_mask":
            ...
            threshold = 0.5
            mask = self.cloud_detector.get_cloud_probability_maps(
                np.moveaxis(np.clip(img, 0, 10000) / 10000, 0, -1)[None, ...]
            )[0, ...]
            mask[mask < threshold] = 0
            mask = gaussian_filter(mask, sigma=2).astype(np.float32)
        elif mask_type == "s2cloudless_mask":
            mask = self.cloud_detector.get_cloud_masks(
                np.moveaxis(np.clip(img, 0, 10000) / 10000, 0, -1)[None, ...]
            )[0, ...]
    ...
    def __getitem__(self, pdx):  # get the time series of one patch
        ...
        if "S1" in self.modalities:
            s1 = [
                read_img(os.path.join(self.root_dir, img))
                for img in self.paths[pdx]["S1"]
            ]
            s1_dates = [
                img.split("/")[-1].split("_")[5] for img in self.paths[pdx]["S1"]
            ]
            sample["S1"] = s1
        ...
        if "S2" in self.modalities:
            s2 = [
                read_img(os.path.join(self.root_dir, img))
                for img in self.paths[pdx]["S2"]
            ]
            s2_dates = [
                img.split("/")[-1].split("_")[5] for img in self.paths[pdx]["S2"]
            ]
