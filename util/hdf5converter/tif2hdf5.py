...
def extract_ROI_tile_patch_index(filename):
    ROI, tile, patch = map(
        re.split("_|.tif", filename.split("/")[-1]).__getitem__, [1, 2, -2]
    )
...
def process_sample_to_hdf5(hdf5_file, batch, verbose=0):
    if "S2" in batch:
        ...
        create_hdf5_group(
            hdf5_file, os.path.join(ROI, tile, f"{ROI}_{tile}_patch_{patch}", "S2")
        )
        ...
        with h5py.File(hdf5_file, "a", libver="latest") as f:
            ...
            f[group].create_dataset(
                "S2",
                data=s2.numpy().astype(np.uint16),
                compression="gzip",
                compression_opts=9,
            )
            ...
            f[group].create_dataset(
                "cloud_mask", data=cloud_mask, compression="gzip", compression_opts=9
            )

            # Date per observation
            f[group].create_dataset(
                "S2_dates",
                data=[date[0] for date in batch["S2_dates"]],
                compression="gzip",
                compression_opts=9,
            )
    ...
    if "S1" in batch:
        ...
        create_hdf5_group(
            hdf5_file, os.path.join(ROI, tile, f"{ROI}_{tile}_patch_{patch}", "S1")
        )
        ...
        with h5py.File(hdf5_file, "a", libver="latest") as f:
            ...
            f[group].create_dataset(
                "S1", data=s1, compression="gzip", compression_opts=9
            )

            # Date per observation
            f[group].create_dataset(
                "S1_dates",
                data=[date[0] for date in batch["S1_dates"]],
                compression="gzip",
                compression_opts=9,
            )
...
def main(args):
    dataset = SEN12MSCRTS_to_hdf5(
        args.root_source, split=args.split, region=args.region
    )
