...
def pathify(path_list):
    return [os.path.join(*path[0].split("/")[-6:]) for path in path_list]
...
if __name__ == "__main__":
    ...
    export_data_path = os.path.join(
        dirname, "precomputed"
    )  # e.g. ...'/3_all_train_vary_s2cloudless_mask.npy'
    vary = (
        "random" if split != "test" else "fixed"
    )  # whether to vary samples across epoch or not
    n_epochs = (
        1 if vary == "fixed" or sample_type == "generic" else 30
    )  # if not varying dates across epochs, then a single epoch is sufficient
    max_samples = int(1e9)

    shuffle = False
    if (
        export_data_path is not None
    ):  # if exporting data indices to file then need to disable DataLoader shuffling, else pdx are not sorted (they may still be shuffled when importing)
            ...
            if sample_type == "generic":
                ...
                if export_data_path is not None:
                    if sample_type == "cloudy_cloudfree":
                        ...
                        data_pairs[adj_pdx] = {
                            "input": patch["input"]["idx"],
                            "target": patch["target"]["idx"],
                            "coverage": {
                                "input": patch["input"]["coverage"],
                                "output": patch["output"]["coverage"],
                            },
                            "paths": {
                                "input": {
                                    "S1": pathify(patch["input"]["S1 path"]),
                                    "S2": pathify(patch["input"]["S2 path"]),
                                },
                                "output": {
                                    "S1": pathify(patch["target"]["S1 path"]),
        ...
        if sample_type == "generic":
            ...
            if export_data_path is not None:
                ...
                if os.path.isdir(export_data_path):
                    export_here = os.path.join(
                        export_data_path,
                        f"{sample_type}_{input_t}_{split}_{region}_{ds.cloud_masks}.npy",
                    )
                else:
    ...
    print(
        f"The grand average variance of S2 samples in the {split} split is: {torch.mean(torch.tensor(collect_var))}"
    )
